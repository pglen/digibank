...
/* =====[ zmalloc.c ]=========================================================
   ...
   Revisions:
      ...
      0.00  sep.29.2017     Peter Glen      Calc sum flag
...
// Cheater malloc. Decorate as follows: (literals in single quotes ')
// xxxx 'a' 'b' 'c' 'd' mmmmmmm 'e' 'f' 'g' 'h'
// xxxx is 4 bytes of length, mmmmmm is the requested memory

//////////////////////////////////////////////////////////////////////////
// Expose the following for testing memory corruption

void *zmalloc_zarr[POOLSIZE] = {NULL};
int  zmalloc_sum_on = (1==0);
int  zmalloc_verbose_on = (1==0);
...

{
    ...
    ret += calc_sum((const char*)zmalloc_zarr, sizeof(zmalloc_zarr));
...

{
    ...
    if(zlinearr[0] == -1)
        {
        memset(zmalloc_zarr, 0, sizeof(zmalloc_zarr)); 
    ...
    *(mem2 + 4 + msize + sizeof(int) + 3) = 'h';
    ...
    if(zlast < POOLSIZE)
        ...
        for(loop = 0; loop < zlast; loop++)
            {
            //printf("Finding slot\n");
            if(zmalloc_zarr[loop] == (void*)0)
                {   
                zmalloc_zarr[loop] = ret;
        ...
        if(loop == zlast)
            {
            printf("zmalloc: Increase zlast memory pool size.\n");
            }
        }
...
// Iterate all known allocations, free them. use it on emergency exit to
// clean up, or in cases where you are confident no aloated memory will
// be accessed.
...

{
    if(mem == NULL)
        {
        printf("Attempting to free NULL at line %d file %s\n",
                zlastline, zlastfile);
        return;
        }
...

{
    ...
    if(aline == -1)
        {
        printf("zmalloc: Trying to free unallocated memory at line %d %s (0x%p)\n",
                zlastline, zlastfile, mem);
        return;
        } 
...

{
    int ret = 0;
    for(int loop = 0; loop < zlast; loop++)
        if(zmalloc_zarr[loop] != NULL)
            {
            fprintf(fp, "zmalloc: Memory leak on %s at line %d (0x%p)\n", 
                            zfnamearr[loop], zlinearr[loop], zmalloc_zarr[loop]); 
            ret++;
            }
    return ret; 
}
