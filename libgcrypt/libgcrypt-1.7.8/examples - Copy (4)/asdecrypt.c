...
int main(int argc, char** argv)
{
    ...
    if(use_stdin)
        ...
        char *err_str;
        sstr  = decode_rsa_cyph(fbuf2, &flen2, &err_str);
        if(!sstr)
            {
        ...
        {    
        flen = strlen(test_str);
        sstr = zalloc(flen + 1);
        strcpy(sstr, test_str);
        }
    else
       {
        FILE* lockf2 = fopen(infile, "rb");
        if (!lockf2) {
            xerr2("fopen() on '%s' failed.", infile);
        }
        
        /* Read and decrypt the key pair from disk. */
        flen2 = getfsize(lockf2);
        zline2(__LINE__, __FILE__);
        char* fbuf2 = zalloc(flen2 + 1);
        if (!fbuf2) {
            xerr("malloc: could not allocate plain text buffer");
        }
        if (fread(fbuf2, flen2, 1, lockf2) != 1) {
            xerr("fread() on input data failed");
        }
        fclose(lockf2);
        //dump_mem(fbuf2, flen2);
