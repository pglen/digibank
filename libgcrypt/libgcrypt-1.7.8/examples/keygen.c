...
int main(int argc, char** argv)
{
    ...
    snprintf(ttt, sizeof(ttt), "%4d/%02d/%02d %02d:%02d:%02d", 
               tmm->tm_year + 1900, tmm->tm_mon + 1, tmm->tm_mday,
                tmm->tm_hour, tmm->tm_min, tmm->tm_sec );
    
    char rrr[32];
    gcry_randomize(rrr, sizeof(rrr), GCRY_STRONG_RANDOM);
    //rrr[sizeof(rrr)-1] = '\0';
    err = gcry_sexp_build(&diba_keys, NULL, 
                "(diba-key (date %s) (key-id %b) ) %S",  ttt, sizeof(rrr), rrr, rsa_keypair);
    print_sexp(diba_keys);
...
// See if keysize has more than one bit set
