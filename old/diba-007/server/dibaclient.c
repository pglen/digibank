...
static int     handshake(int sock, cchar *sstr, int slen, char *buff, int rlen);
static void    decrypt_session_key(gcry_sexp_t *privk, char *buffer, int len);
...

{
    ...
    int ret;
    //ret = handshake(clsock, "session", 7, buffer, sizeof(buffer));
    
    ret = handshake(clsock, "key", 3, buffer, sizeof(buffer));
    if(strncmp(buffer, okstr, STRSIZE(okstr)) != 0)
        {
        printf("Server did does not accept key command.\n");
        ret = handshake(clsock, "close", 5,  buffer, sizeof(buffer));
        zautofree();
        exit(4);
        }
    ret = handshake(clsock, mykey, sizeof(mykey), buffer, sizeof(buffer));
    if(strncmp(buffer, okstr, STRSIZE(okstr)) != 0)
        {
    ...
    //printf("Rand sent: '%s'\n", sumstr);
    // Test echo
    ret = handshake(clsock, sumstr, strlen(sumstr), buffer, sizeof(buffer));
    
    #if 0
    // Test for badcommand response
    ret = handshake(clsock, "badcomand", 9, buffer, sizeof(buffer));
    if(ret <= 0)
        {
        zautofree();
        exit(5);
    ...
    zfree(randstr3); 
    ret = handshake(clsock, sumstr3, strlen(sumstr3), buffer, sizeof(buffer));
    if(ret <= 0)
        {
        zautofree();
        exit(5);
    ...
    decrypt_session_key(&privk, buffer, ret);
    
    int rlen2 = rand() % 32 + 24;
    char *randstr2 = zrandstr_strong(rlen2); 
    char *sumstr2 = zstrmcat(0, "check ", querystr, NULL); 
    zfree(randstr2); 
    ret = handshake(clsock, sumstr2, strlen(sumstr2), buffer, sizeof(buffer));
    if(ret <= 0)
        {
        zautofree();
        exit(5);
...
void    decrypt_session_key(gcry_sexp_t *privk, char *buffer, int len)
