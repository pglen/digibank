...
static char descstr[] = "Symmetric encryption utility.";
static char usestr[]  = "blueencrypt [options]\n";
...
opts opts_data[] = {
        ...
        'd',  "stdout",    NULL, NULL,  0, 0, &use_stdout, 
        "-d             --stdout            - use stdout as output",
...
int main(int argc, char** argv)
{
    ...
    if (!file_buf) {
        //fclose(fp);
        xerr2("blueencrypt: could not allocate file buffer for encryption.");
        }
    ...
    while(1==1)
        ...
        if(use_stdin)
            ...
            if (fread(file_buf, curr_len, 1, fp) != 1) {
                zfree(file_buf);
                fclose(fp);
                xerr2("blueencrypt: Cannot read file for encrypting.");
                }
            }
               
        bluepoint3_decrypt(file_buf, curr_len, thispass, strlen(thispass));    
        ...
        if(use_stdout)
            ...
            if (fwrite(file_buf, curr_len, 1, fp2) != 1) {
                ...
                xerr2("blueencrypt: Cannot write encrypted file.");
                }
            }
         prog += curr_len;
