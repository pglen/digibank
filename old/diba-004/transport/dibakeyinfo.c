...

{
    ...
    if (fread(rsa_buf, rsa_len, 1, lockf) != 1) {
        xerr("Read on public key failed");
    }
    ...
    if(check)
        ...
        if(shax)
            ...
            int olen;
            char *hash_str = hash_sexp(pk, &olen);
            unsigned int plen2;
...

{
    ...
    if (fread(rsa_buf, rsa_len, 1, lockf) != 1) {
        xerr("Read on private key failed.");
    }
    ...
    if (err) {
        printerr(err, "encrypt");
        xerr("Failed to create private key sexp.");
        }
    ...
    if(!nocrypt)
        ...
        err = gcry_cipher_decrypt(fish_hd, (unsigned char*) buff3, 
                                  plen3, NULL, 0);
        if (err) {
            xerr("Could not decrypt with TWOFISH");
            }
...

{
    ...
    if(thispass[0] == '\0')
        ...
        if(!nocrypt) 
            ...
            if(ret < 0)
                {
                xerr("Error on entering pass, no keys are decoded.\n");
                }
            }
        }
    else
