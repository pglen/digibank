...

{
    ...
    *( (short *)(outptr+outlen2)) = (short) sizeof(pub_hash);
    outlen2 += sizeof(short); 
    memcpy(outptr+outlen2, pub_hash, sizeof(pub_hash));
    outlen2 += sizeof(pub_hash); 
    ...
    *( (short *)(outptr+outlen2)) = (short) (plen2 & 0xffff) + sizeof(int);
    outlen2 += sizeof(short); 
    gcry_randomize(outptr + outlen2, sizeof(int), GCRY_STRONG_RANDOM);
    outlen2 += sizeof(int); 
    memcpy(outptr + outlen2, buff2, plen2); 
    outlen2 += plen2;
    ...
    zline2(__LINE__, __FILE__);
    char *tmp_buf = zalloc(keylen + sizeof(int));
    ...
    zfree(infile); zfree(outfile); zfree(keyfile);    
    zfree(dummy);
    zleak();
    return 0;
}
