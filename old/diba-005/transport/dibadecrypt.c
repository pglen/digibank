   ...
   if (argc - nn != 2 && strlen(keyfile) == 0) {
    ...
    if(thispass[0] == '\0' && !nocrypt)
        ...
        if(thispass[0] == '@')
            ...
            if(fp == NULL) {
                xerr2("dibadecrypt: Cannot open pass file '%s'\n", passfile);
                }
            ...
            if (!pass_buf) {
                fclose(fp);
                xerr2("dibadecrypt: could not allocate password file buffer");
                }
            if (fread(pass_buf, pass_len, 1, fp) != 1) {
                fclose(fp);
                xerr2("dibadecrypt: Cannot read password from file.");
                }
    ...
    if(privk == NULL)
        {                           
        xerr2("dibadecrypt: No private key present in '%s'", keyfile);
        }
    //print_sexp(privk);
    ...
    if(use_stdin)
    ...
    else if(infile[0] != '\0')
       {
        ...
        if (!lockf2) {
            xerr2("dibadecrypt: Cannot open input file '%s'.", infile);
        }
    ...
    #ifndef TEST_BLUE
    int bret = bluepoint2_decrypt(data_buf, data_len, keypass, strlen(keypass));
    #endif
    ...
    if(strlen(outfile))
        ...
        if(!outf)
            {
            xerr2("dibadecrypt: Cannnot open outfile");
            }
        }
    int retf = fwrite(outptr, outlen2, 1, outf); 
    if(retf != 1)
        {
        xerr2("dibadecrypt: Cannot write to file");
        }
    if(outf != stdout)
        {
        fclose(outf);
