...
int main(int argc, char** argv)
{
    ...
    //if(keysize % 2 )
    //    {
    //    xerr2("dibakeygen: Keysize must be even %d", keysize);
    //    }
        
    if(num_bits_set(keysize) != 1)
        {
        xerr2("dibakeygen: Keysize must be a power of two. ( ... 1024, 2048, 4096 ...)");
        } 
    ...
    if(access(fname, F_OK) >= 0 && !force)
        {
        xerr2("dibakeygen: File already exists, use different name or delete the file or use -f (--force) option.");
        }
    ...
    if (err) {
        ...
        xerr2("dibakeygen: Failed to create rsa params");
    }
    
    err = gcry_pk_genkey(&rsa_keypair, rsa_parms);
    if (err) {
        printerr(err, "create keypair");
        xerr2("dibakeygen: Failed to create rsa key pair");
    }
    ...
    if(thispass[0] == '\0' && !nocrypt)
        ...
        if(ret < 0)
            {
            xerr2("dibakeygen: Error on entering pass, no keys are written.\n");
            }
        }
    else
        {
        ...
        if(thispass[0] == '@')
            ...
            if(fp == NULL) {
                xerr2("dibakeygen: Cannot open pass file '%s'\n", passfile);
                }
            ...
            if (!pass_buf) {
                fclose(fp);
                xerr2("dibakeygen: malloc: could not allocate password file buffer");
                }
            if (fread(pass_buf, pass_len, 1, fp) != 1) {
                fclose(fp);
                xerr2("dibakeygen:: Cannot read password from file.");
                }
    ...
    if(err)
        xerr2("dibakeygen: Cannot create sexpr: '%s'\n", gcry_strerror (err));
    ...
    if(write_pubkey(&glib_pub, fname2) < 0)
        xerr2("dibakeygen: Could not write pubic key");
    ...
    if (!rsa_buf) {
        xerr2("dibakeygen: malloc: could not allocate rsa buffer");
    }
    ...
    if(rsa_len == 0)
        {
        xerr2("dibakeygen: Cannot sprint keypair");
        }
    ...
    if(nocrypt)
        ...
        err = gcry_cipher_encrypt(aes_hd, (unsigned char*) rsa_buf, 
                                  rsa_len, NULL, 0);
        if (err) {
            xerr2("dibakeygen: Could not encrypt with AES");
            }
        ...
        err = gcry_cipher_encrypt(fish_hd, (unsigned char*) rsa_buf, 
                                  rsa_len, NULL, 0);
        if (err) {
            xerr2("dibakeygen: Could not encrypt with TWOFISH");
            }
